...
Defines the server-rendered Hiccup/Tailwind operator console: login, dashboard, dispute queue/detail actions, manual exception attachment, counterparty pages, correlation review, ingestion settings, form parsing, and tenant session lookup.
...
- Upstream: Hiccup, `drw.domain.disputes`, `drw.domain.exceptions`, `drw.domain.correlations`, `drw.domain.ingestion-sources`, `drw.domain.counterparties`, and `drw.tenants.store`.
...
- `test/drw/e2e_api/ui_flow_test.clj` verifies the first tenant operator flow, correlation acceptance, and ingestion settings/pull-now over a running Pedestal server.
