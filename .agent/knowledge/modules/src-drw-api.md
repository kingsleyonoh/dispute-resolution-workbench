...
- Upstream: `drw.http.json`, `drw.security.hmac`, `drw.tenants.store`, `drw.domain.disputes`, `drw.domain.exceptions`, `drw.domain.correlations`, `drw.domain.ingestion-sources`, `drw.domain.counterparties`, `clojure.string`.
...
- Workbench handler and E2E tests cover tenant isolation, validation errors, duplicate exception source refs, illegal transitions, cross-tenant 404s, correlation accept/reject terminal behavior, ingestion source/run controls, and Hub-routed HMAC exception ingestion.
