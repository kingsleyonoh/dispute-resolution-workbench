...
Provides the Pedestal service map, route table, JSON helpers, API interceptors, health endpoint, tenant endpoints, and server-rendered root page.

## Key files

- `src/drw/http/server.clj` - Pedestal service-map, server start, and server stop helpers.
- `src/drw/http/routes.clj` - route table for root, health, tenant registration, tenant profile, and tenant key rotation routes.
...
- Upstream: `io.pedestal.http`, `io.pedestal.interceptor`, `drw.api.tenants`, `drw.tenants.store`, `drw.ui.pages`, `hiccup2.core`.
- Downstream: `src/drw/core.clj` starts the server via `requiring-resolve`.

## Tests

- Route tests verify wired tenant routes and invalid dev-route flag handling.
- Interceptor tests verify request-id propagation, auth failures, tenant binding, and rate limiting.
- E2E tests start a real Pedestal server and request health, root, and tenant lifecycle endpoints.
